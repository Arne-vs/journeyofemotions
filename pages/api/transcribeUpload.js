...
export default async function handler(req, res) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    return res.status(500).json({ error: "missing_api_key", message: "OPENAI_API_KEY not set" });
  }

  try {
    const buf = await readBuffer(req);
    const size = buf?.length || 0;
    if (!size) return res.status(400).json({ error: "empty_audio", message: "No audio bytes received" });

    const MAX_BYTES = 6 * 1024 * 1024; // ~6MB guard
    if (size > MAX_BYTES) {
      return res.status(413).json({ error: "payload_too_large", message: `Audio ${(size/1024/1024).toFixed(2)}MB` });
    }
